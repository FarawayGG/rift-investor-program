...
#[derive(Accounts)]
#[instruction(params: WithdrawExcessTokensParams)]
pub struct WithdrawExcessTokens<'info> {
    #[account(mut,
...
pub struct WithdrawExcessTokensParams;

impl<'info> WithdrawExcessTokens<'info> {
    pub fn handle(
        ctx: &mut Context<'_, '_, 'info, 'info, Self>,
        _: WithdrawExcessTokensParams,
    ) -> Result<()> {
