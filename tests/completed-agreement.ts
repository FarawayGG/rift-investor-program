...
describe("completed-agreement", () => {
  ...
  let companyPaymentTokenAccount: anchor.web3.PublicKey;
  let companyProjectTokenAccount: anchor.web3.PublicKey;
  ...
  before(async () => {
    companyPaymentTokenAccount = await createAssociatedTokenAccount(
      program.provider.connection,
      program.provider.wallet.payer,
      paymentTokenMint,
      companyWallet.publicKey
    );
    companyProjectTokenAccount = await createAssociatedTokenAccount(
      program.provider.connection,
      program.provider.wallet.payer,
      projectTokenMint,
      companyWallet.publicKey
  ...
  it("Deposits stablecoins by investors", async () => {
    ...
    // check agreement (should be fully funded)
    const agreementAccount = await program.account.agreement.fetch(agreement);
    assert.ok(agreementAccount.holdDurationStart > new anchor.BN(0));
  });

  it("Deposits stablecoins by non-investor", async () => {
    const tx = await program.methods.depositStablecoins({ amount: new anchor.BN(150) }).accounts({
      agreement,
      investor: null,
      destinationTokenAccount: companyPaymentTokenAccount,
  ...
  it("Withdraws tokens by investors", async () => {
    await sleep(1000);
  ...
  it("Withdraws funds", async () => {
    const tx = await program.methods.withdrawFunds({}).accounts({
      agreement,
